...
$sql = "SELECT t.name as task_name, t.date, t.completed,t.file, t.project_id, p.name as project_name, p.id FROM `tasks` t JOIN `projects` p ON t.project_id = p.id WHERE t.user_id=1 ";
$result=mysqli_query($con,$sql);
if(!$result){
$error=mysqli_error($con);
print("Ошибка MYSQL".$error);
...
// echo "</pre>";
  ...
  if(!empty($_POST) && empty($errors)){
    ...
    mysqli_stmt_bind_param($stmt, "ssisii", $name, $date,  $completed, $file, $project_id, $user_id);
      ...
      <main class="content__main">
        ...
        <form class="form"  method="post" autocomplete="off" enctype="multipart/form-data">
          <div class="form__row">
            ...
            <input class="form__input <?if (isset($errors['name'])) echo ' form__input--error';?>" type="text" name="name" id="name" value="<?= htmlspecialchars(getPostVal('name')); ?>" placeholder="Введите название">
            ...
            <select class="form__input form__input--select " name="project_id" id="project" >
                ...
                <?php foreach ($projects_arr as $key=>$value) : ?>
                  <?php $project_id=$value['id'];
                        $project_name=$value['name'] ;
                  ?>
                  <option value="<?= $project_id; ?>" <?if($project_id==getPostVal('project_id'))echo ' selected' ;?>><?= htmlspecialchars($project_name); ?></option>
              <?php endforeach; ?>
          ...
          <div class="form__row">
            ...
            <input class="form__input form__input--date  <?if (isset($errors['date'])) echo ' form__input--error';?>" type="text" name="date" id="date" value="" placeholder="Введите дату в формате ГГГГ-ММ-ДД">
